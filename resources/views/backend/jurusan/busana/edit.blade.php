...
@section("content")
    <div id="main" class="main-content flex-1">
       <x-titlepage title="data jurusan busana" quote="ubah data jurusan busana di smkn 1 gedangan " isRoute="true" nameRoute="Kembali" href="{{ route('busana.index') }}"></x-titlepage>
        <div class="w-full p-5">
            <form id="form" action="{{ route('busana.update',[Crypt::encrypt($busana->id)]) }}" class="mt-4 w-full flex flex-col" method="POST" enctype="multipart/form-data">
                @csrf
                @method("PUT")
                    <div class="grid grid-cols-12 space-y-5 gap-2">
                        <div class="col-span-7 ">
                            <div>
                                ...
                                <x-text-input type="text" id="judul" class="block mt-1 w-full" name="judul" :value="old('judul',$busana->judul)" required  />
                        ...
                        <div class="col-span-7">
                            <div>
                                ...
                                <x-text-input type="text" id="nama_kaprog" class="block mt-1 w-full" name="nama_kaprog" :value="old('nama_kaprog',$busana->nama_kaprog)" required  />
                        ...
                        <div class="col-span-7">
                            <div>
                                ...
                                <x-text-input type="text" id="ket_kaprog" class="block mt-1 w-full" name="ket_kaprog" :value="old('ket_kaprog',$busana->ket_kaprog)" required  />
                        ...
                        <div class="col-span-7">
                            <div>
                                ...
                                <x-text-input type="file" id="photo_kaprog" class="block border border-black mt-1 w-full" name="photo_kaprog" :value="old('photo_kaprog',$busana->photo_kaprog)"  />
                                <x-input-error :messages="$errors->get('photo')" class="mt-2" />
                                @if (file_exists(public_path('storage/' . $busana->photo_kaprog)) && $busana->photo_kaprog)
                                <p class="mt-3">photo kaprog saat ini : </p>
                                <img class="w-72 h-auto" src="{{ asset("storage/" . $busana->photo_kaprog) }}" alt="">
                                @else
                                <p class="mt-3">photo kaprog saat ini : </p>
                        ...
                        <div class="col-span-10">
                         ...
                         <div class="col-span-7">
                            <div>
                                ...
                                <x-text-input type="file" id="photo" class="block mt-1 w-full border border-black" name="photo" :value="old('photo',$busana->photo)"/>
                                <x-input-error :messages="$errors->get('photo')" class="mt-2" />
                                @if (file_exists(public_path('storage/' . $busana->photo)) && $busana->photo)
                                <p class="mt-3">photo Jurusan: </p>
                                <img class="w-72 h-auto" src="{{ asset("storage/" . $busana->photo) }}" alt="">
                                @else
                                <p class="mt-3">photo Jurusan: </p>
                                <div class="bg-gray-200 w-44 h-52 grid place-content-center">
                                <span>No Image</span> <!-- Pesan fallback -->
