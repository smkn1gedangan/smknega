    ...
    <div id="main" class="main-content flex-1 bg-gray-100 md:pt-20 md:pl-6 md:mt-2">
        ...
        <div class="my-5 w-3/4 pl-10">
            ...
            @if (file_exists(public_path('img/jurusan/' . $boga->photo)) && $boga->photo)
                           <img src="{{ asset('img/jurusan/' . $boga->photo) }}" class="object-cover w-full rounded-t-lg h-40 md:h-auto md:w-auto md:rounded-none md:rounded-s-lg" alt="{{ $boga->photo }}">
           @else
           ...
           <div class="prose mb-3 text-gray-900 dark:text-gray-400 mt-6">{!!$boga->konten!!}</div>
            ...
            <div class="items-center ml-6 pb-6">
                ...
                <a href="{{ route('boga.edit', [Crypt::encrypt($boga->id)]) }}" class="bg-yellow-500 hover:bg-yellow-600 dark:text-yellow-500 ml-4 text-white py-2.5 px-4 rounded-md shadow-md transition-all duration-200">Edit Jurusan Tata Boga</a>
            </div>
        </div>
@endsection
