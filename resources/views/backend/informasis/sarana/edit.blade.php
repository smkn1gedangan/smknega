...
@section("content")
    <div id="main" class="main-content flex-1 ">
        <x-titlepage title="data Sarana Prasarana" quote="ubah data Sarana Prasarana smkn 1 gedangan " isRoute="true" nameRoute="Back" href="{{ route('sarana.index') }}"></x-titlepage>
        <div class="w-full p-5">
            <form id="form" action="{{ route('sarana.update',[Crypt::encrypt($sarana->id)]) }}" class="mt-4 w-full flex flex-col" method="POST" enctype="multipart/form-data">
                @csrf
                @method("PUT")
