...
@section("content")
    <div id="main" class="main-content flex-1 bg-gray-100 md:pt-20 md:pl-6 md:mt-2">
        <x-title-create-dashboard class="lowercase">edit visi misi sekolah</x-title-create-dashboard>
        <div class="w-full">
            <form id="form" action="{{ route('visi.update',[Crypt::encrypt($visi->id)]) }}" class="mt-4 w-full flex flex-col" method="POST" enctype="multipart/form-data">
                @csrf
                @method("PUT")
                ...
                <div class="w-11/12 mt-4 mb-8">
                    <button type="submit"
                            class="inline-block px-6 py-2 text-white bg-yellow-500 rounded-lg shadow-md hover:bg-yellow-600 focus:bg-yellow-500 focus:outline-none">
                        Ubah Visi Misi
                    </button>
                </div>
            </form>
