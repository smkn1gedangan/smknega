...
@section("content")
    <div id="main" class="main-content flex-1 ">
        <x-titlepage title="data potensi unggulan" quote="ubah data potensi unggulan smkn 1 gedangan " isRoute="true" nameRoute="Back" href="{{ route('potensi.index') }}"></x-titlepage>
        <div class="w-full p-5">
            <form id="form" action="{{ route('potensi.update',[Crypt::encrypt($potensi->id)]) }}" class="mt-4 w-full flex flex-col" method="POST" enctype="multipart/form-data">
                @csrf
                @method("PUT")
