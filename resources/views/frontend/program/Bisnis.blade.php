...
<section class="w-full flex md:justify-center flex-wrap pt-32">
    <div class="flex flex-col w-full p-2 sm:w-5/6 md:w-4/5 md:items-center md:p-8  lg:w-3/5">
        <x-heading-profil class="w-full">bisnis sekolah</x-heading-profil>
            <div class="swiper mySwiper w-full flex justify-center">
                <div class="swiper-wrapper">
                    @foreach ($bisnisPhoto as $bp)
                        @if (file_exists(public_path('storage/' . $bp->photo)) && $bp->photo)
                        <div class="swiper-slide ">

                            <img class="relative left-1/2 -translate-x-1/2 w-11/12 sm:w-5/6 sm:h-64 rounded-md object-cover my-5" src="{{ asset('storage/' . $bp->photo) }}" alt="">
                        </div>
                        @else
            ...
            <p data-aos="fade-up" data-aos-duration="1000" data-aos-anchor-placement="top-bottom" class="mb-3 w-full text-xs md:text-base text-left md:text-center font-normal text-gray-800 dark:text-gray-400 mt-2">ditulis oleh {{$bisnis->penulis->name}}  {{$bisnis->updated_at->diffForHumans()}}</p>
    </div>
