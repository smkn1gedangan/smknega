...
<section class="w-full flex justify-center flex-wrap pt-32">
    <div class="w-full lg:w-full p-2 flex flex-col items-center">
        ...
        @if (file_exists(public_path('img/jurusan/' . $sija->photo)) && $sija->photo)
                    <div data-aos="fade-up" data-aos-duration="1000" data-aos-anchor-placement="top-bottom"  class="img-fixed my-5 rounded-md w-11/12 sm:w-5/6 md:w-3/4 lg:w-4/5 min-h-64 md:min-h-96"
                        style="background-image: url('{{ asset("img/jurusan/" . $sija->photo) }}');">
                    </div>
                    @else
                        <div data-aos="fade-up" data-aos-duration="1000" data-aos-anchor-placement="top-bottom"  class="bg-gray-200 w-11/12 sm:w-5/6 h-96 my-5 grid place-content-center text-xs">
