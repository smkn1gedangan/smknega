...
<section class="w-full flex md:justify-center flex-wrap pt-32">
    <div class="md:w-full p-2 flex flex-col items-center">
        ...
        @if (file_exists(public_path('img/jurusan/' . $busana->photo)) && $busana->photo)
                <img class="w-11/12 sm:w-5/6 md:w-3/4 lg:w-4/5 h-52 sm:h-64 md:h-auto rounded-md object-cover my-5" src="{{ asset("img/profil/" . $busana->photo) }}" alt="">
                    @else
    ...
    <div class="flex flex-col w-full p-4 sm:w-5/6 md:w-4/5 md:items-center md:p-8  lg:w-3/5">
            ...
            <p class="mb-3 w-full text-left font-normal text-gray-800 dark:text-gray-400 mt-6">ditulis oleh {{$busana->penulis->name}}  {{$busana->created_at->diffForHumans()}}</p>
    </div>
