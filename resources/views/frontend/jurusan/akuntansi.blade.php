...
<section class="w-full flex justify-center flex-wrap pt-20 md:pt-28">
    <div class="w-full lg:w-full p-2 flex flex-col items-center">
        ...
        @if (file_exists(public_path('img/jurusan/' . $akuntansi->photo)) && $akuntansi->photo)
                    <div class="img-fixed my-5 rounded-md object-cover object-center w-11/12 sm:w-5/6 md:w-3/4 lg:w-4/5 min-h-64 md:min-h-96 md:h-auto"
                        style="background-image: url('{{ asset("img/jurusan/" . $akuntansi->photo) }}');">
                    </div>
                    @else
    ...
    <div class="flex flex-col w-full p-2 sm:w-5/6 md:items-center md:p-8  lg:w-3/5">
            ...
            <p data-aos="fade-up" data-aos-duration="1000" data-aos-anchor-placement="top-bottom" class="mb-3 w-full text-xs md:text-base text-left font-normal text-gray-800 dark:text-gray-400 mt-2">ditulis oleh {{$akuntansi->penulis->name}}  {{$akuntansi->created_at->diffForHumans()}}</p>
    </div>
