...
<section class="w-full flex justify-center flex-wrap pt-20 md:pt-28">
    <div class="w-full lg:w-full p-2 flex flex-col items-center">
        ...
        @if (file_exists(public_path('img/jurusan/' . $tkr->photo)) && $tkr->photo)
                    < <img src="{{ asset("img/jurusan/" . $tkr->photo) }}" class="relative my-5 object-cover object-center rounded-md w-11/12 sm:w-5/6 md:w-3/4 lg:w-4/5 h-auto"/>
                    @else
    ...
    <div class="flex flex-col w-full p-2 sm:w-5/6 md:items-center md:p-8  lg:w-3/5">
        <div class="w-full flex flex-wrap justify-center gap-2 ">
            ...
            <img src="{{ asset('img/jurusan/' . $tkr->photo_kaprog) }}" class="object-cover w-64 sm:w-64 md:w-52 rounded-t-lg md:rounded-s-lg mt-8" alt="{{ $tkr->photo_kaprog }}">
            @else
            ...
            <div class="flex flex-col justify-center mx-4">
                <h1 class="text-base text-center md:text-left font-semibold mt-6 text-gray-800 dark:text-gray-100 capitalize">{{$tkr->nama_kaprog}}</h1>
                <p class="text-center md:text-left">sebagai</p>
                <h1 class="text-base text-center md:text-left font-semibold mb-2 text-gray-800 dark:text-gray-100 capitalize">{{$tkr->ket_kaprog}}</h1>
            </div>
        </div>
            ...
            <p data-aos="fade-up" data-aos-duration="1000" data-aos-anchor-placement="top-bottom" class="mb-3 w-full text-xs md:text-base text-left md:text-center font-normal text-gray-800 dark:text-gray-400 mt-2">ditulis oleh {{$tkr->penulis->name}}  {{$tkr->created_at->diffForHumans()}}</p>
    </div>
