...
<section class="w-full flex md:justify-center flex-wrap pt-20 md:pt-28 ">
    <div class="flex flex-col w-full sm:w-5/6  md:w-4/5 md:items-center lg:w-3/5 p-2">
        ...
        @if (file_exists(public_path('img/profil/' . $sejarah->photo)) && $sejarah->photo)
            ...
            <div data-aos="fade-up" data-aos-duration="1000" data-aos-anchor-placement="top-bottom"  class="prose mb-3 text-gray-900 dark:text-gray-400 mt-2 w-full">
                {!! $sejarah->konten !!}
            </div>
            <p data-aos="fade-up" data-aos-duration="1000" data-aos-anchor-placement="top-bottom"  class="mb-3 w-full text-xs md:text-base text-left md:text-center font-normal text-gray-800 dark:text-gray-400 mt-2">ditulis oleh {{$sejarah->penulis->name}}  {{$sejarah->updated_at->diffForHumans()}}</p>
    </div>
