...
<section class="w-full flex md:justify-center flex-wrap pt-20 md:pt-28">
    <div class="relative flex flex-col w-full p-2 sm:w-5/6 md:w-4/5 md:items-center md:p-8  lg:w-3/5">
        ...
        @if (file_exists(public_path('img/profil/' . $logo->photo)) && $logo->photo)
            ...
            <p data-aos="fade-up" data-aos-duration="1000" data-aos-anchor-placement="top-bottom" class="mb-3 w-full text-xs md:text-base text-left md:text-center font-normal text-gray-800 dark:text-gray-400 mt-2">ditulis oleh {{$logo->penulis->name}}  {{$logo->created_at->diffForHumans()}}</p>
    </div>
